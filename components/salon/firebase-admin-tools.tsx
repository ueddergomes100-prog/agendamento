...
export function MembershipForm(){
  ...
  return <section className="panel"><h2>Acesso da equipe</h2><p>A pessoa precisa criar uma conta primeiro. Informe o identificador da conta para conceder acesso a este salão.</p><form className="form-stack" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);setBusy(true);try{await api.rpc('admin_member',{salon_id:catalog.salon.id,user_id:f.get('uid'),role,status:f.get('status'),...(role==='PROFESSIONAL'?{professional_id:f.get('professional')}:{})});notice('Permissões atualizadas.');}catch(e){notice((e as Error).message);}finally{setBusy(false);}}}><label>Identificador da conta<input name="uid" required maxLength={128}/></label><label>Função<select value={role} onChange={e=>setRole(e.target.value)}><option value="RECEPTIONIST">Recepção</option><option value="MANAGER">Gerência</option><option value="PROFESSIONAL">Profissional</option><option value="FINANCE">Financeiro</option></select></label>{role==='PROFESSIONAL'&&<label>Perfil profissional<select required name="professional">{catalog.professionals.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}<label>Acesso<select name="status"><option value="ACTIVE">Ativo</option><option value="INACTIVE">Suspenso</option></select></label><button className="btn" disabled={busy}>{busy?'Salvando…':'Salvar acesso'}</button></form></section>;
}
