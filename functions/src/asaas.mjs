...
export async function financialDispatch(db,uid,action,p,c,makeClient=asaasClient){
  ...
  if(action==='asaas_status'){
    ...
    const a=account.data(),credentialed=!!(c.enabled&&c.apiKey),subaccountsApproved=c.subaccountsApproved===true;
    return {configured:credentialed,platformReady:credentialed&&subaccountsApproved,subaccountsApproved,environment:c.environment,monthlyCents:subscriptionPlans[0].monthlyCents,plans:subscriptionPlans,account:a?{id:a.id,status:a.status,payout:a.payout||null,ownerName:a.ownerName}:null,subscription:billing.data()?.public||null,payments:ps.docs.map(d=>d.data()).filter(d=>d.environment===c.environment),transfers:ts.docs.map(d=>{const t=d.data();return{id:d.id,status:t.status,valueCents:t.valueCents,date:t.date,environment:t.environment};}).filter(t=>t.environment===c.environment)};
  }
  requireAsaas(c);
