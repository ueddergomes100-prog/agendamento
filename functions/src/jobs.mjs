...
export const deliverNotifications=onSchedule({schedule:'every 5 minutes',region:'southamerica-east1',maxInstances:1,timeoutSeconds:120,memory:'256MiB'},async()=>{
  ...
  for(const snapshot of batch.docs)await deliverOutbox(db,snapshot);
});

export const deliverBookingUpdate=onDocumentCreated({document:'salons/{salonId}/outbox/{eventId}',region:'southamerica-east1',maxInstances:2,retry:true},async event=>{
 if(event.data)await deliverOutbox(getFirestore(),event.data);
});

async function deliverOutbox(db,snapshot){
    const job=await db.runTransaction(async tx=>{
      const current=await tx.get(snapshot.ref),data=current.data();
      if(data?.status!=='PENDING'||(data.scheduledAt?.toMillis()||0)>Date.now()||(data.leaseUntil?.toMillis()||0)>Date.now())return null;
    ...
    try{
      ...
      if(stale||!relationship.data()?.preferences?.push||devices.empty){await snapshot.ref.update({status:'SKIPPED',reason:stale?'STALE':'NO_PUSH_CONSENT_OR_DEVICE',leaseUntil:null});return;}
      const tokens=devices.docs.map(d=>d.data().token),link=`https://agendamento-taupe-mu.vercel.app/?salon=${encodeURIComponent(publicSalon.data()?.slug||'')}&view=agenda`;
      ...
      await snapshot.ref.update({status:transient&&(job.attempts||0)<3?'PENDING':transient?'FAILED':'SENT',leaseUntil:null,scheduledAt:Timestamp.fromMillis(Date.now()+300000),successCount:sent.successCount,failureCount:sent.failureCount,updatedAt:Timestamp.now()});
    }catch(error){
      logger.warn('push_delivery_failed',{eventId:snapshot.id,code:error.code||'internal'});
      await snapshot.ref.update({status:(job.attempts||0)<3?'PENDING':'FAILED',leaseUntil:null,scheduledAt:Timestamp.fromMillis(Date.now()+300000),updatedAt:Timestamp.now()});
    }
}
