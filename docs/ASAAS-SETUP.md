...
O app não armazena chave de API no navegador, no Firestore ou no bundle web. A configuração inteira fica no Secret Manager do Firebase em `ASAAS_CONFIG`.
...
6. Só depois defina `enabled: true` e promova para produção.
